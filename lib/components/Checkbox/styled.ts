...
export const styles = (
  p: StyledCheckboxProps & { theme: Theme },
) => css`
position: relative;
...
align-items: center;
height: ${p.theme.checkbox.size};
width: ${p.theme.checkbox.size};
background-color: ${p.theme.checkbox.backgroundColor};
border: ${p.borderless ? 'none' : p.theme.checkbox.border};
border-radius: ${p.theme.checkbox.borderRadius};
outline: none;
transform: ${p.theme.checkbox.transform};
transition: ${getCSSForTransition(
  p.theme.checkbox.transition,
)};
cursor: pointer;
...
${
  ...
  background-color: ${
    p.theme.checkbox.disabled.backgroundColor
  };
  transition: ${getCSSForTransition(
